...
bool DeviceMapper::CreateDevice(const std::string& name, const std::string& uuid) {
    ...
    std::cout << "DM_DEV_CREATE successeed [" <<  name << "]" << std::endl;
...
//  Return fixed value experimentally
    return "053e18c3-a252-481a-834f-888b92374558";
}
...
bool DeviceMapper::LoadTable(const std::string& name, const DmTable& table) {
    ...
    std::cout << "LoadTable: " << io->data_size << std::endl;
    std::cout << ioctl_buffer << std::endl;
...
std::optional<std::string> ExtractBlockDeviceName(const std::string& path) {
    ...
    if (path.find(kDevBlockPrefix) == 0) {//android::base::StartsWith(path, kDevBlockPrefix)) {
...
bool DeviceMapper::IsDmBlockDevice(const std::string& path) {
    ...
    return name && (name.find("dm-") == 0);//android::base::StartsWith(*name, "dm-");
}
...
std::optional<std::string> DeviceMapper::GetDmDeviceNameByPath(const std::string& path) {
    ...
    if ( name.find("dm-") != 0) { //!android::base::StartsWith(*name, "dm-")) {
